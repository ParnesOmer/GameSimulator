...
void StateTrooper::update() {
    ...
    while (now < tick_end && state != VehicleState::PARKED) {
        ...
        if (moveDistance >= distanceToTarget) {
            ...
            if (!returningHome && currentTargetIdx != -1) {
                std::cout << "[DEBUG] StateTrooper arrived at warehouse " << warehousesToVisit[currentTargetIdx]
                          << " at " << TimeConverter::timeToString(now)
                          << ", Position: " << targetPos.toString() << std::endl;
                state = VehicleState::STOPPED;
                warehousesToVisit.erase(warehousesToVisit.begin() + currentTargetIdx);
                moveToNextWarehouse();
            } else if (returningHome) {
                std::cout << "[DEBUG] StateTrooper returned to start warehouse " << startWarehouseName
                          << " at " << TimeConverter::timeToString(now)
                          << ", Position: " << startWarehousePos.toString() << std::endl;
...
std::string StateTrooper::broadcastState() const {
    ...
    oss << "StateTrooper " << getName() << " at " << getPosition().toString()
        << ", State: " << getState();
    if (!returningHome && currentTargetIdx != -1) {
        oss << ", Heading to warehouse" << warehousesToVisit[currentTargetIdx];
    } else if (returningHome) {
